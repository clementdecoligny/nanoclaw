...
TAG="${1:-latest}"
# Default to docker to match the host, which spawns containers via a hardcoded
# `docker` binary (CONTAINER_RUNTIME_BIN in src/container-runtime.ts). Upstream
# defaults this to Apple's `container` CLI; on this install that default builds
# nothing and fails with "container: command not found". Still overridable via
# the environment for anyone running a different runtime.
