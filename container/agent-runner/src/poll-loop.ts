 ...
 */
function formatMessagesWithCommands(messages: MessageInRow[], nativeSlashCommands: boolean, cwd: string): string {
  const parts: string[] = [];
  ...
  for (const msg of messages) {
    if (nativeSlashCommands && (msg.kind === 'chat' || msg.kind === 'chat-sdk')) {
      ...
      // Dispatch natively only for admin commands and passthrough commands
      // backed by a real command file (`<cwd>/.claude/commands/<name>.md` or
      // the user dir). A passthrough command with no backing file (e.g. a
      // behavior defined only in CLAUDE.local.md like `/weekly`) would be
      // silently dropped by the SDK, so it falls through to standard XML
      // formatting and the agent interprets it from its instructions.
  ...
  try {
    for await (const event of query.events) {
      ...
      if (event.type === 'init') {
        ...
        // The SDK auto-compacted the conversation. After compaction the model
        // often drops the learned `<message to="…">` wrapping discipline (the
        // destinations are still in the system prompt, but the behavioral
        // pattern is summarized away). Inject a reminder back into the live
        // query so the next turn re-anchors on the destination model. Only
        // when there's >1 destination — single-destination groups have a
        // fallback that works without wrapping. See qwibitai/nanoclaw#2325.
        // Not applicable to task runs: they deliver via send_message only.
        if (!routing.taskRun) {
          const destinations = getAllDestinations();
          if (destinations.length > 1) {
            ...
            query.push(
              `<system>Context was just compacted. Reminder: you have ${destinations.length} destinations (${names}). ` +
                `Use <message to="name"> blocks to address them. Bare text goes to the scratchpad fallback only.</system>`,
            );
          }
        }
      }
...
function handleEvent(event: ProviderEvent, _routing: RoutingContext): void {
  switch (event.type) {
    ...
    case 'progress':
      ...
      break;
    case 'compacted':
      // Logged by the provider with token detail; nothing more to add here.
      break;
  }
}
