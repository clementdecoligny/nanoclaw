...
export const sendDocument: McpToolDefinition = {
  tool: {
    ...
    inputSchema: {
      type: 'object' as const,
      properties: {
        // Static description: this object is built at module load, before the
        // session DBs exist. Calling destinationList() here would open the
        // inbound DB at import time and throw SQLITE_CANTOPEN. The handler
        // still lists the live destinations when `to` is missing or unknown.
        to: { type: 'string', description: 'Destination name. Required.' },
