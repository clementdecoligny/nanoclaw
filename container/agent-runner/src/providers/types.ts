...
export type ProviderEvent =
  ...
  | { type: 'activity' }
  /**
   * The provider auto-compacted the conversation. Bookkeeping only — never a
   * turn result. The poll-loop uses it to re-anchor the agent on the
   * destination model, which compaction tends to summarize away.
   * See qwibitai/nanoclaw#2325.
   */
  | { type: 'compacted' };
