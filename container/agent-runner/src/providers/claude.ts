...
import { writeToolStatusMessage } from '../status-feedback.js';
...
const preToolUseHook: HookCallback = async (input) => {
  ...
  writeToolStatusMessage(toolName);
    ...
    async function* translateEvents(): AsyncGenerator<ProviderEvent> {
      ...
      for await (const message of sdkResult) {
        ...
        if (message.type === 'system' && message.subtype === 'init') {
          ...
          yield { type: 'activity' };
          // Separate bookkeeping event so the poll loop can re-anchor the
          // agent on its destinations — compaction summarizes away the
          // <message to="…"> wrapping discipline. See qwibitai/nanoclaw#2325.
