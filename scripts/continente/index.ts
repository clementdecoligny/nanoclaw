...
async function syncDelivery(): Promise<void> {
  const pantryPath = path.join(GROUP_PATH, 'inventory/pantry.md');
  const consumptionPath = path.join(GROUP_PATH, 'inventory/consumption-log.md');

  console.log('Fetching latest Continente order...');
  ...
  // Output structured JSON for Pepa to use when updating pantry.md
...
} else if (cmd === 'sync-delivery') {
  syncDelivery().catch((err) => {
    console.error('sync-delivery failed:', err.message);
    process.exit(1);
  });
} else {
  console.error('Usage: npx tsx scripts/continente/index.ts [prepare|execute|sync-delivery]');
  process.exit(1);
}
