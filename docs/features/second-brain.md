...
| Work email | Off limits, unchanged from persona. Personal Gmail only. |
...
**12 months.** Clément's top-priority retrieval questions are year-scale ("how
many doctor visits for Tom last year"), which a 90-day window could not answer.
Calendar first (he creates placeholders for every planned event, so it is already
a hand-built life log), then Gmail.
