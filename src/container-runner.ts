...
export function buildMounts(
  ...
  // Local-only: upstream removed this mount, but multiple groups share state here.
...
export async function buildAgentGroupImage(agentGroupId: string): Promise<void> {
  ...
  // Shared with the unit-tested helper so apt/npm/pip generation has one
  // definition. `pip` is local-only — upstream has no Python in the base image.
  let dockerfile = buildPackageDockerfile({ apt: aptPackages, npm: npmPackages, pip: pipPackages });
