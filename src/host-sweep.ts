...
async function sweepSession(session: Session): Promise<void> {
  ...
  try {
    ...
    // spawn-kill loop. Complements ORPHAN_CLAIM_GRACE_MS in decideStuckAction,
    // which covers later ticks where the container is slow to first heartbeat.
...
async function sweepTelegramStatusMessage(inDb: Database.Database, session: Session, errorText: string): Promise<void> {
