...
import { CLAIM_STUCK_MS, ORPHAN_CLAIM_GRACE_MS, startHostSweep, stopHostSweep } from './host-sweep.js';
...
beforeEach(() => {
  ...
  //
  // Claim age must land in the window CLAIM_STUCK_MS < age < ORPHAN_CLAIM_GRACE_MS.
  // This fork also carries the orphan-claim guard in decideStuckAction, which
  // skips no-heartbeat claims OLDER than the grace window as leftovers from a
  // previous run. Upstream's original 2h age would be swallowed by that guard,
  // so this test could never observe the justWoke behavior it exists to pin.
  //
  // Derived from the constants rather than hardcoded, and biased to the bottom
  // of the window: the claim keeps aging in real time while the test runs, and
  // a value near the top drifts past the grace window on a slow machine.
  const claimAgeMs = CLAIM_STUCK_MS + (ORPHAN_CLAIM_GRACE_MS - CLAIM_STUCK_MS) / 4;
  writeSessionMessage(AG, SESS, { id: 'm-1', kind: 'chat', timestamp: now(), content: '{"text":"hi"}' });
  seedStaleClaim('m-1', claimAgeMs);
});
