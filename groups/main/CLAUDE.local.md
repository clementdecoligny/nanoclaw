...
This is the user's only copy of this project — no external backup besides the nightly `scripts/backup-db.sh` push to `origin`. That script also writes `/workspace/project/data/git-branch-hygiene.json` (only when there's something to report) listing:
- `merged` — branches fully merged into `main`, safe to delete
- `stale` — branches not merged and untouched 2+ days. The user always finishes what they start same-day, so anything older than that is forgotten, not in-progress work — still worth a nudge to review rather than auto-deleting, since it might hold something not yet merged.

If not already scheduled, set up a daily check:

```
schedule_task(
  prompt: "Read /workspace/project/data/git-branch-hygiene.json if it exists. Message the user a short summary: merged branches as 'safe to delete' with the git branch -d command, stale branches as 'review — not touched in Nd, probably forgotten' without deleting them. Then delete the file.",
  schedule_type: "cron",
  schedule_value: "30 9 * * *",
  script: "test -f /workspace/project/data/git-branch-hygiene.json && echo '{\"wakeAgent\": true}' || echo '{\"wakeAgent\": false}'"
)
```

Runs shortly after the nightly backup (00:00) so the file is ready. Never delete branches yourself — only report; the user decides.
